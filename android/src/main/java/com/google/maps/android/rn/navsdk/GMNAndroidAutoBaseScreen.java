...
@DoNotStrip
public abstract class GMNAndroidAutoBaseScreen extends Screen
    implements SurfaceCallback, IGMNNavigationViewController {
  ...
  private final GMNAndroidAutoBaseScreen screenInstance = this;

  public void onNavigationReady(boolean ready) {
    mNavigationInitialized = ready;
  }

  public GMNAndroidAutoBaseScreen(@NonNull CarContext carContext) {

    super(carContext);

    GMNNavAutoModule.setModuleReadyListener(
        () -> {
          mAndroidAutoModuleInitialized = true;
          registerControllersForAndroidAutoModule();
        });

    GMNNavModule.setModuleReadyListener(
        () -> {
          mNavModuleInitialized = true;
          GMNNavModule.getInstance().registerNavigationReadyListener(this::onNavigationReady);
        });
  ...
  private void registerControllersForAndroidAutoModule() {
    if (mAndroidAutoModuleInitialized && mMapViewController != null) {
      GMNNavAutoModule.getInstance()
          .androidAutoNavigationScreenInitialized(mMapViewController, this);
    }
  }

  private void unRegisterControllersForAndroidAutoModule() {
    if (mAndroidAutoModuleInitialized) {
      GMNNavAutoModule.getInstance().androidAutoNavigationScreenDisposed();
    }
  }
  ...
  protected void sendCustomEvent(String type, ReadableMap data) {
    GMNNavAutoModule.getInstance().onCustomNavigationAutoEvent(type, data);
  }
