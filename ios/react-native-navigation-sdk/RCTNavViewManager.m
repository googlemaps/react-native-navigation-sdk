...
- (void)logDebugInfo: (NSString *) info {        
    [self sendCommandToReactNative:@"logDebugInfo" args: info];
}
...
RCT_EXPORT_METHOD(setTrafficIncidentCards : (nonnull NSNumber *)reactTag isEnabled : (BOOL)isEnabled) {
  dispatch_async(dispatch_get_main_queue(), ^{
    [viewController setTrafficIncidentCards:isEnabled];
  });
}
...
RCT_EXPORT_METHOD(setMapType :(nonnull NSNumber *)reactTag mapType: (NSInteger)mapType) {
  dispatch_async(dispatch_get_main_queue(), ^{
    [self logDebugInfo: @"setMApType native"];
