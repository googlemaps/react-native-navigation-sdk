...
- (void)mapView:(GMSMapView *)mapView didTapInfoWindowOfMarker:(GMSMarker *)marker {
  [callbacks logDebugInfo:@"didTapOverlay"];
  [callbacks onMarkerInfoWindowTapped:marker];
}

- (BOOL)mapView:(GMSMapView *)mapView didTapMarker:(GMSMarker *)marker {
  [callbacks logDebugInfo:@"onOverlayClick"];
...
- (void)initializeNavigation {
  [callbacks logDebugInfo:@"termsAccepted"];
                                        ...
                                        companyName:companyName
                                           callback:^(BOOL termsAccepted) {
                                             if (termsAccepted) {
                                               ...
                                               [callbacks logDebugInfo:@"termsRejected"];
               ...
               callback:^(GMSRouteStatus routeStatus) {
                 switch (routeStatus) {
                   case GMSRouteStatusOK:
                     [callbacks logDebugInfo:[NSString stringWithFormat:@""]];
                     break;
                   case GMSRouteStatusCanceled:
                     [callbacks
                         logDebugInfo:[NSString stringWithFormat:@"Route guidance cancelled."]];
                     break;

                   default:
                     [callbacks
                         logDebugInfo:[NSString stringWithFormat:@"Error starting guidance: %zd",
                                                                 routeStatus]];
                 }

                 [callbacks logDebugInfo:[NSString stringWithFormat:@"Error %d", (int)routeStatus]];
...
- (void)setTrafficIncidentCards:(BOOL)isEnabled {
...
- (void)startUpdatingLocation {
  [callbacks logDebugInfo:@"startUpdatingLocation"];
  [_mapView.roadSnappedLocationProvider startUpdatingLocation];
}

- (void)stopUpdatingLocation {
  [callbacks logDebugInfo:@"stopUpdatingLocation"];
