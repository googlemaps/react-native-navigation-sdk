...
RCT_EXPORT_METHOD(setDestination : (nonnull NSDictionary *)waypoint : (NSDictionary *)
                      routingOptions : (NSDictionary *)displayOptions : (NSString *)routeToken : (
                          RCTPromiseResolveBlock)resolve : (RCTPromiseRejectBlock)reject) {
  NSArray *waypoints = @[ waypoint ];
  [self setDestination:waypoints:routingOptions:displayOptions:routeToken:resolve:reject];
}

RCT_EXPORT_METHOD(setDestinations : (nonnull NSArray *)waypoints : (NSDictionary *)
                      routingOptions : (NSDictionary *)displayOptions : (NSString *)routeToken : (
                          RCTPromiseResolveBlock)resolve : (RCTPromiseRejectBlock)reject) {
#if DEBUG
  NSLog(@"[NavModule] Setting destinations with routeToken: %@", routeToken);
  NSLog(@"[NavModule] Waypoints: %@", waypoints);
  NSLog(@"[NavModule] Routing options: %@", routingOptions);
#endif
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    if (routeToken != nil && ![routeToken isEqualToString:@""]) {
