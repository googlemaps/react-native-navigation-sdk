...
- (instancetype)init {
  if (self = [super init]) {
    ...
    const auto &localDefaultProps = *std::static_pointer_cast<const NavViewProps>(defaultProps);
...
- (void)unregisterView {
  ...
  if (!currentProps.nativeID.empty()) {
    ...
    [[NavViewModule viewControllersRegistry] removeObjectForKey:@(nativeIDAsInteger)];
  }
}

- (void)updateProps:(Props::Shared const &)props oldProps:(Props::Shared const &)oldProps {
  const auto &oldViewProps = *std::static_pointer_cast<NavViewProps const>(_props);
  ...
  if (!_initialized && !newViewProps.nativeID.empty()) {
    ...
    [NavViewModule viewControllersRegistry][@(nativeIDAsInteger)] = _viewController;
  }
  ...
  if (oldViewProps.navigationViewStylingOptions != newViewProps.navigationViewStylingOptions) {
    [_viewController
        setStylingOptions:convertFollyDynamicToId(newViewProps.navigationViewStylingOptions)];
  }
  ...
  if (newViewProps.followingPerspective) {
    [_viewController setFollowingPerspective:@(newViewProps.followingPerspective)];
  }
  ...
  [_viewController setTrafficIncidentCardsEnabled:newViewProps.trafficIncidentsCardEnabled];
  ...
  if (!_initialized &&
      (oldViewProps.initialCameraPosition.target.lat !=
       ...
       oldViewProps.initialCameraPosition.bearing != newViewProps.initialCameraPosition.bearing ||
       oldViewProps.initialCameraPosition.tilt != newViewProps.initialCameraPosition.tilt ||
       oldViewProps.initialCameraPosition.zoom != newViewProps.initialCameraPosition.zoom)) {
    GMSMutableCameraPosition *position = [[GMSMutableCameraPosition alloc] init];

    position.target = CLLocationCoordinate2DMake(newViewProps.initialCameraPosition.target.lat,
                                                 newViewProps.initialCameraPosition.target.lng);
...
- (void)handlePolylineClick:(GMSPolyline *)polyline {
  ...
  NavViewEventEmitter::OnPolylineClick result = {
      ...
      polyline.strokeColor
          ? [[ObjectTranslationUtil hexStringFromColor:polyline.strokeColor] UTF8String]
          : nil,
...
- (void)handlePolygonClick:(GMSPolygon *)polygon {
  ...
  NavViewEventEmitter::OnPolygonClick result = {
      ...
      [ObjectTranslationUtil isIdOnUserData:polygon.userData] ? [polygon.userData[0] UTF8String]
                                                              : "",
      polygon.fillColor ? [[ObjectTranslationUtil hexStringFromColor:polygon.fillColor] UTF8String]
                        : nil,
      (float)polygon.strokeWidth,
      polygon.strokeColor
          ? [[ObjectTranslationUtil hexStringFromColor:polygon.strokeColor] UTF8String]
          : nil,
      0,  // strokeJointType
...
- (void)handleCircleClick:(GMSCircle *)circle {
  NavViewEventEmitter::OnCircleClick result = {
      ...
      circle.fillColor ? [[ObjectTranslationUtil hexStringFromColor:circle.fillColor] UTF8String]
                       : nil,
      (float)circle.strokeWidth,
      circle.strokeColor
          ? [[ObjectTranslationUtil hexStringFromColor:circle.strokeColor] UTF8String]
          : nil,
      (float)circle.radius,
